...
Com a utilização de uma tabela \emph{match+action} \todo{use emph em vez de texit para destacar palavras em outras línguas.} é possível 
...
e o Atomix \cite{atomixio}\todo{FEITO.OpenReplica é um framework para replicacao. Chubby é uma aplicação, um serviço de locks 
distribuído. Então, essencialmente, estão em categorias diferentes. Se quiser ficar com a primeira, cite, em vez do chubby, o atomix, que implementa o raft}. 
...
em consideração que dentre esses $n$ elementos existirão alguns que falharão.\todo{FEITO. entendo o que quer dizer, mas tem um problema aqui. maioria é bem definido. quorum é que é variável. acho que os conceitos estão invertidos aqui.}
...
\todo{FEITO foi movido um paragráfo para cima. está faltando algo aqui.}
...
\lstinputlisting[language=c++, caption={parser.p4}]{code/p4-example/parser.p4}
...
\lstinputlisting[language=c++, caption={table.p4}]{code/p4-example/table.p4}
...
\lstinputlisting[language=c++, caption={action.p4}]{code/p4-example/action.p4}
...
\lstinputlisting[language=c++, caption={program.p4}]{code/p4-example/program.p4}
...
\lstinputlisting[language=c,caption={Cabeçalho do P4 Paxos}, label={cod:p4-header}]{code/p4xos/explanation/headers.p4}
...
\lstinputlisting[language=c,caption={P4 Paxos coordinator}]{code/p4xos/explanation/coordinator.p4}
